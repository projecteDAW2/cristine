...
function displayFinalScreen() {
  ...
  ranking.data_user.push({ id: 9, nickname: "new-user", points: scoreFinal });
...
function displayRank() {
  ...
  for (let y = 0; y < ranking.data.length; y++) {
    let rowTable = table.insertRow(y);
    let objectLength = Object.getOwnPropertyNames(ranking.data[y]).length;
    for (let h = 0; h < objectLength; h++) {
      ...
      if (h === 0) {
        ...
        column.innerHTML = Object.values(ranking.data[y])[h];
      }
  ...
  if (ranking.data_user !== null && ranking.data_user.length > 0) {
    for (let y = 0; y < ranking.data_user.length; y++) {
      debugger;
      let rowTable = table.insertRow(ranking.data.length);
      let objectLength = Object.getOwnPropertyNames(ranking.data_user).length;
      for (let h = 0; h <= objectLength; h++) {
        let column = rowTable.insertCell(h);
        ...
        if (h === 0) {
          ...
          column.innerHTML = Object.values(ranking.data_user[0])[h];
        }
        column.style.color = "white";
        column.style.fontSize = "50px";
      }
